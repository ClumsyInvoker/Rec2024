...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            # 转化成tensor
            user_id = torch.LongTensor([user_id]).to(self.device)
            history_items = torch.LongTensor(history_items + [0] * (self.max_length - len(history_items))).to(
                self.device)
            history_items_len = torch.LongTensor(
                [1] * len(history_items) + [0] * (self.max_length - len(history_items))).to(self.device)
            target_item_id = torch.LongTensor([target_item_id]).to(self.device)
            user_features = torch.FloatTensor(user_features.values).to(self.device)
            item_features = torch.FloatTensor(item_features.values).to(self.device)
...
class PTCRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            item_pos_feedback = self.item_history[target_item_id][max(0, item_positive_behavior_offset + 1 -
                                                                      self.feedback_max_length):item_positive_behavior_offset + 1]
            item_neg_feedback = self.item_history[target_item_id][max(0, item_negative_behavior_offset + 1 -
                                                                      self.feedback_max_length):item_negative_behavior_offset + 1]
            item_pos_feedback = torch.LongTensor(item_pos_feedback + [0] * (self.feedback_max_length - len(item_pos_feedback))).to(self.device)
            item_pos_feedback_len = torch.LongTensor([1] * len(item_pos_feedback) + [0] * (self.feedback_max_length - len(item_pos_feedback))).to(self.device)
            item_neg_feedback = torch.LongTensor(item_neg_feedback + [0] * (self.feedback_max_length - len(item_neg_feedback))).to(self.device)
            item_neg_feedback_len = torch.LongTensor([1] * len(item_neg_feedback) + [0] * (self.feedback_max_length - len(item_neg_feedback))).to(self.device)
