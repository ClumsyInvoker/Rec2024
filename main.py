...
if __name__ == '__main__':
    ...
    config = {'embed_dim': args.embed_dim,
              'dim_config': {'item_id': itemnum+1, 'user_id': usernum+1, 'item_feature': args.embed_dim, 'user_feature': args.embed_dim},
              'device': args.device}
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step in range(num_batch):  # tqdm(range(num_batch), total=num_batch, ncols=70, leave=False, unit='b'):
            ...
            u, seq, pos, neg, seq_len = torch.from_numpy(u).to(args.device), torch.from_numpy(seq).to(args.device), \
                                        torch.from_numpy(pos).to(args.device), torch.from_numpy(neg).to(args.device), \
                                        torch.from_numpy(seq_len).to(args.device)

            # pos_logits, neg_logits = model(u, seq, pos, neg)
            pos_logits = model(pos, seq, seq_len, None, None)
            neg_logits = model(neg, seq, seq_len, None, None)
