...
from data.MyDataset import CDNDataset
...
parser.add_argument('--enable_user_behavior', action='store_true', default=False)
...
if __name__ == '__main__':
    # dataset
    dataset_train = CDNDataset(data_dir='data/' + args.dataset,
                                                    max_length=args.maxlen, mode='train', device=args.device)
    dataset_train_cold = CDNDataset(data_dir='data/' + args.dataset,
                                                    max_length=args.maxlen, mode='train_cold', device=args.device)
    dataset_valid = CDNDataset(data_dir='data/' + args.dataset,
                                                    max_length=args.maxlen, mode='val', neg_num=args.num_test_neg_item, device=args.device)
    dataset_test = CDNDataset(data_dir='data/' + args.dataset,
    ...
    config['enable_behavior_embedding'] = getattr(args, 'enable_user_behavior', False)

    if args.model_name == "CDN":
        model = CDN(config).to(args.device)
    else:
    ...
    dataloader_full = DataLoader(dataset_train, batch_size=args.batch_size, shuffle=True)
    train_full_loader_iter = iter(dataloader_full)
    gamma = args.gamma

    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        alpha = 1 - (epoch / (args.num_epochs * gamma)) * (epoch / (args.num_epochs * gamma))


        if args.inference_only: break  # just to decrease identition
        dataloader = DataLoader(dataset_train_cold, batch_size=args.batch_size, shuffle=True)
        step = 0
        epoch_loss = 0.0
        ...
        for data in train_loop:
            ...
            try:
                data_m = next(train_full_loader_iter)
            except StopIteration:
                train_full_loader_iter = iter(dataloader_full)
                data_m = next(train_meta_loader_iter)

            user_id_m, history_items_m, history_items_len_m, target_item_id_m, \
                user_features_m, item_features_m, label_m, cold_item_m = data_m

            # logits = model(user_id, target_item_id, history_items, history_items_len, user_features, item_features)
            logits_r = model(user_id, target_item_id, history_items, history_items_len, user_features, item_features, cold_item, mode='r')
            logits_m = model(user_id_m, target_item_id_m, history_items_m, history_items_len_m, user_features_m, item_features_m, cold_item_m, mode='m')

            adam_optimizer.zero_grad()
            # indices = np.where(target_item_id.cpu() != 0)
            # loss = bce_criterion(logits[indices], label[indices])
            loss_r = bce_criterion(logits_r, label)
            loss_m = bce_criterion(logits_m, label_m)
            loss = alpha * loss_m + (1-alpha) * loss_r
